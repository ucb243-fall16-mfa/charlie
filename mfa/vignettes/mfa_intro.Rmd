...
$$
And finally, compute the PCA of the combined table X^'. The new principle components are factors. 
Note that in the `mfa` package, we use the SVD formulations of PCA instead of eigen decomposition, to achieve greater numerical stability. We also use SVD, as the the right singular matrix contains information about the loadings of variables on factors. This is useful in interpretation of results.
...
We further examine the features of the `mfa` package in other documents on [plotting](), the [shiny app](), and [contributions]().