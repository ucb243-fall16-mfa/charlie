...
# The first column is just wine IDs, and the last few columns are supplementary physical data. Columns 2-54 are the
# actual sets of ratings.
data1 = data[,2:54]
```
The `mfa` class requires a `sets` argument, specifying which columns belong to each table respectively. `sets` is a list of vectorrs specifying the column indices. The following `sets` argument shows the columns for each table.
```{r]}
